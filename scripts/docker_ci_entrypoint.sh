...
# Detect PyQt5 availability — on linux/arm64 the Dockerfile deliberately
# strips PyQt5 + pytest-qt because no manylinux2014_aarch64 wheel exists.
# When absent, Phase 3 (GUI smoke) is skipped and Phase 4 (full regression)
# ignores every test module that touches a Qt binding.
HOTWIRE_PYQT_AVAILABLE=1
if ! python -c "import PyQt5" >/dev/null 2>&1; then
    HOTWIRE_PYQT_AVAILABLE=0
    echo "PyQt5 binding: NOT installed (arm64 in-container path or host-without-PyQt5)"
else
    python -c "from PyQt5.QtCore import QT_VERSION_STR; print('PyQt5/Qt', QT_VERSION_STR)"
fi
echo "QT_QPA_PLATFORM=${QT_QPA_PLATFORM}"
echo "OpenV2G binary: $(ls -la hotwire/exi/codec/OpenV2G 2>/dev/null || echo 'missing!')"

# All GUI-touching test files (import PyQt5 / pytest-qt / qtbot at module
# scope). When PyQt5 is absent we --ignore each of these so pytest's
# collection step doesn't fail on import.
GUI_TEST_FILES=(
    tests/test_gui_worker_reuse.py
    tests/test_gui_integration.py
    tests/test_gui_dual_scenarios.py
    tests/test_gui_smoke.py
    tests/test_session_replay_real_hw.py
    tests/test_preflight_wizard.py
    ...
    tests/test_session_compare_panel.py
    tests/test_live_pcap_viewer.py
    tests/test_session_tools_panel.py
    tests/test_hw_runner_panel.py
    tests/test_session_replay.py
)
PYTEST_GUI_IGNORE=()
if [ "$HOTWIRE_PYQT_AVAILABLE" = "0" ]; then
    for f in "${GUI_TEST_FILES[@]}"; do
        PYTEST_GUI_IGNORE+=("--ignore=$f")
    done
fi
...
# Skipped entirely when no Qt binding is installed (arm64 path).
if [ "$HOTWIRE_CI_GUI" = "1" ] && [ "$HOTWIRE_PYQT_AVAILABLE" = "1" ]; then
    banner "Phase 3/4 — GUI headless smoke (PyQt5 offscreen)"
    step gui_smoke python -m pytest -v --tb=short \
        ...
        tests/test_interface_picker.py \
        tests/test_interface_status_dock.py
elif [ "$HOTWIRE_CI_GUI" = "1" ]; then
    banner "Phase 3/4 — GUI headless smoke (SKIPPED: no Qt binding)"
    echo ">>> [gui_smoke] SKIPPED — PyQt5 not installed on this platform"
fi
...
if [ "$HOTWIRE_CI_REGRESSION" = "1" ]; then
    ...
    # wraps everything. On arm64 (no PyQt5), GUI test files are added to
    # --ignore via PYTEST_GUI_IGNORE so pytest collection doesn't crash
    # on module-level `import PyQt5`.
