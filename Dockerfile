...
RUN apt-get update && apt-get install -y --no-install-recommends \
    ...
    gcc g++ python3-dev \
    git \
    `# Qt5 build toolchain — required to source-build PyQt5 on` \
    `# platforms with no manylinux wheel (notably linux/arm64 on` \
    `# Apple Silicon Docker Desktop). On x86_64 hosts pip would` \
    `# normally pick the manylinux wheel and skip these, but` \
    `# having them present means the image builds correctly` \
...
#
# Two filters applied to requirements.txt before pip install:
#
#   1. pypcap — Docker can't access PLC modems anyway and the latest
#      pypcap doesn't build cleanly on Python 3.12. The hotwire code
#      imports it lazily and falls back to simulation when missing.
#
#   2. PyQt5 on arm64 — Riverbank Computing publishes manylinux2014
#      wheels only for x86_64 + win_amd64. On linux/arm64 (Apple
#      Silicon Docker Desktop, Raspberry Pi, GHA arm runners) pip
#      falls into a multi-hour sip-build source-compile that exhausts
#      runner resources during "Preparing metadata". GUI tests in
#      tests/ skip cleanly with `pytest.importorskip("PyQt5")` when
#      the binding is absent, exactly mirroring how the host fallback
#      path behaves on a machine without PyQt5 installed.
#
# Both filters preserve the requirements.txt source-of-truth used by
# host installs; only the in-container requirements.docker.txt is
# pruned.
COPY requirements.txt ./
ARG TARGETARCH
RUN set -eux; \
    grep -v '^pypcap' requirements.txt > requirements.docker.txt; \
    if [ "${TARGETARCH:-amd64}" = "arm64" ]; then \
        echo "[docker-build] arm64 target: stripping PyQt5 + pytest-qt from in-container deps"; \
        echo "[docker-build]   reason: no manylinux2014_aarch64 wheel exists upstream;"; \
        echo "[docker-build]   pytest-qt fails plugin load without a Qt binding present."; \
        sed -i '/^PyQt5/d; /^pyqt5/d; /^pytest-qt/Id' requirements.docker.txt; \
    fi; \
