...
thread, and the override flow reaches the PauseController.
...
def test_apply_override_flows_to_pause_controller(qtbot):
    ...
    # The other 6 fields must also be present — otherwise an intercept
    # that only sees a partial override could drop mandatory DIN
    # DC_EVSEStatus fields on the wire.
