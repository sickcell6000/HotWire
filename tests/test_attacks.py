...
def test_discharge_stores_voltage():
    # Post-2026-05 the A2 attack no longer overrides PreChargeRes —
    # the EVSE FSM's default ramp mirrors the PEV's EVTargetVoltage so
    # PreCharge always converges. The user-supplied voltage now lands
    # on every CurrentDemandRes instead. See forced_discharge.py
    # module docstring "Why no PreChargeRes override".
    a = ForcedDischarge(voltage=420)
    assert "PreChargeRes" not in a.overrides
    assert a.overrides["CurrentDemandRes"]["EVSEPresentVoltage"] == 420
...
def test_discharge_apply_and_clear_roundtrip():
    # Post-2026-05: A2 only installs a CurrentDemandRes override.
    pc = PauseController()
    a = ForcedDischarge(voltage=400)
    a.apply(pc)
    cdr = pc.get_override("CurrentDemandRes")
    assert cdr is not None
    assert cdr["EVSEPresentVoltage"] == 400
    a.clear(pc)
    assert pc.get_override("CurrentDemandRes") is None


def test_discharge_override_changes_intercept_output():
    # The lying voltage now lands on CurrentDemandRes (where it
    # actually matters operationally), not PreChargeRes.
    pc = PauseController()
    ForcedDischarge(voltage=999).apply(pc)
    merged = pc.intercept(
        "CurrentDemandRes", {"EVSEPresentVoltage": 400, "EVSEPresentCurrent": 50}
    )
...
def test_two_attacks_touching_different_stages_compose():
    ...
    cdr = pc.get_override("CurrentDemandRes")
    assert cdr is not None
    assert cdr["EVSEPresentVoltage"] == 450
