...
class DualGUIHarness:
    ...
    def reset(self) -> None:
        """Stop workers, clear pause/override state, clear trace buffers."""
        ...
        # Clear pause flags too.
        for stage in self.evse.stage_config._schemas.keys():
            self.evse.pause_controller.set_pause_enabled(stage, False)
        for stage in self.pev.stage_config._schemas.keys():
            self.pev.pause_controller.set_pause_enabled(stage, False)
...
def scenario_3_evse_pause_edit(h: DualGUIHarness) -> tuple[bool, str]:
    """EVSE pauses PreChargeRes, auto-'presses Send' with EVSEPresentVoltage=999.

    We bypass the modal dialog by connecting our own slot to ``pause_hit``
    that calls ``pause_controller.send(...)`` directly — same code path
    a human click would take.
    """
    # Harness is fresh — no reset() needed.

    # Tick the Pause checkbox programmatically via the nav signal.
    h.evse.stage_nav.pause_toggled.emit("PreChargeRes", True)

    # Replace the modal dialog with a headless auto-responder.
    # Disconnect the real handler first (connects to _on_pause_hit).
    try:
        h.evse.signals.pause_hit.disconnect(h.evse._on_pause_hit)
    except TypeError:
        pass

    intercepted: list[tuple[str, dict]] = []

    def auto_release(stage: str, params: dict) -> None:
        intercepted.append((stage, params))
        if stage == "PreChargeRes":
            new_params = dict(params)
            new_params["EVSEPresentVoltage"] = 999
            h.evse.pause_controller.send(new_params)
        else:
            h.evse.pause_controller.send(params)

    h.evse.signals.pause_hit.connect(auto_release)

    h.evse.start_worker()
    h.pump(1.0)
    h.pev.start_worker()

    # PEV's decoded trace should show EVSEPresentVoltage.Value = 999
    ok = h.pump_until(
        lambda: any(
            '"EVSEPresentVoltage.Value": "999"' in t for t in h.pev_traces
        ),
        timeout_s=15.0,
    )
    return ok, (
        f"PreCharge intercepted {len(intercepted)} time(s); "
        f"PEV saw EVSEPresentVoltage=999"
    ) if ok else (
        f"intercepted={len(intercepted)}, "
        "but PEV never decoded EVSEPresentVoltage=999"
    )
...
SCENARIO_REGISTRY = {
    ...
    "3": ("3. EVSE pause-and-edit PreCharge", scenario_3_evse_pause_edit),
