...
def _run_child(scenario: str) -> dict:
    ...
    proc = subprocess.run(
        ...
        # UTF-8 bytes; on Windows with a CJK locale the default text-mode
        # decoder is cp950/cp932/cp949 and chokes on 0xe2-prefixed multi-
        # byte sequences. The artifact runs the same harness on Linux /
        # macOS / Windows reviewers; this keeps it locale-independent.
