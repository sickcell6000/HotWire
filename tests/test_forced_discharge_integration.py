...
def test_forced_discharge_propagates_to_current_demand():
    ...
    # Sanity check: the attack lies only at CurrentDemandRes.
    # PreCharge succeeds via the EVSE FSM's default voltage ramp that
    # mirrors the PEV's EVTargetVoltage — see "Why no PreChargeRes
    # override" in hotwire/attacks/forced_discharge.py. The earlier
    # design overrode PreChargeRes too, but that hung the session in
    # WaitForPreChargeRes whenever the PEV's target voltage differed
    # from the operator-supplied fake voltage.
    assert "PreChargeRes" not in attack.overrides
