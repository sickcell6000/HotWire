...
def test_pev_precharge_timeout_uses_lenient_value():
    """Runtime PreCharge timeout uses the pyPLC-compatible lenient
    value (30 s), not the DIN spec's 7 s — real EVSE DC ramp-up takes
    longer than the spec value on commercial chargers, and HotWire
    follows pyPLC's empirical timeout to avoid false-aborting paper-
    validation runs while the EVSE is mid-ramp.

    The spec constant ``V2G_EVCC_PRE_CHARGE_TIMEOUT_S`` (7 s) is still
    asserted by ``test_timing_constants_match_table_78`` for
    documentation; this test pins the runtime behaviour against the
    lenient constant.
    """
    from hotwire.fsm import fsm_pev
    from hotwire.fsm.din_spec import V2G_EVCC_PRE_CHARGE_TIMEOUT_LENIENT_S
    f = fsm_pev.fsmPev.__new__(fsm_pev.fsmPev)
    f.state = fsm_pev.STATE_WAIT_PRECHARGE_RES
    f.cyclesInState = seconds_to_cycles(V2G_EVCC_PRE_CHARGE_TIMEOUT_LENIENT_S) - 1
    assert not f.isTooLong(), (
        "PreCharge should not time out before the lenient 30 s window"
    )
    f.cyclesInState = seconds_to_cycles(V2G_EVCC_PRE_CHARGE_TIMEOUT_LENIENT_S) + 5
    assert f.isTooLong(), (
        "PreCharge should time out after the lenient 30 s window"
    )
    # Also verify the spec constant is still 7 s (paper / conformance
    # claim — HotWire knows the spec value, intentionally chooses
    # lenient at runtime).
    assert V2G_EVCC_PRE_CHARGE_TIMEOUT_S == 7.0
