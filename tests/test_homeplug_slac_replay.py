...
def test_replay_extracts_real_pev_run_id() -> None:
    ...
    for raw in iter_homeplug_frames(path):
        ...
        if fr is not None and fr.is_slac_param_req() and fr.src_mac == pev_mac:
            # Payload starts at wire byte 19 (after FMI). Layout per
            # pyPLC composeSlacParamReq: APP_TYPE(1) + SEC_TYPE(1) +
            # RunID(8) → payload offsets 0..9, RunID at [2:10].
            if len(fr.payload) >= 10:
                pcap_run_id = fr.payload[2:10]
                break
