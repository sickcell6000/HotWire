...
# --- PreCharge ramping --------------------------------------------------
#
# Volts to add to ``self._precharge_present_voltage`` per PreChargeReq
# we receive. At the FSM tick rate (33 ms) and a typical PEV PreCharge
# poll cadence of ~5 reqs/sec, 25 V/step yields ~125 V/s — fast enough
# for the testbed loop to converge in a couple of seconds even when the
# target is 500 V, slow enough that tracing the ramp on the wire is
# visible in the operator's GUI. pyPLC's modified fsmEvse uses a
# configurable step; we keep it constant here since the testbed has no
# real EVSE-side hardware to constrain the ramp rate.
_PRECHARGE_RAMP_STEP_V = 25.0
...
class fsmEvse:
    ...
    def __init__(
        ...
        # PreCharge voltage-ramping state. Real DC chargers ramp their
        # output voltage gradually from 0 V toward the EV's
        # EVTargetVoltage during PreCharge so the BMS sees a smooth
        # rise rather than an instant step (the latter is associated
        # with contactor weld / inrush damage on real packs). HotWire
        # mirrors that on the EVSE side: each PreChargeRes reports the
        # *current* ramp value, increased by ``_PRECHARGE_RAMP_STEP_V``
        # per request until it reaches the target. Real EVs expect this
        # — they keep sending PreChargeReq until EVSEPresentVoltage
        # matches their EVTargetVoltage. See pyPLC fsmEvse.py for the
        # original implementation we mirror here.
        self._precharge_present_voltage: float = 0.0
        self._precharge_target_voltage: float = 0.0
        # Ongoing-loop counters for CableCheck / ChargeParameterDiscovery.
        # Real chargers send "Ongoing" several times before "Finished" so
        # the EV BMS has time to do its isolation / range checks.
        # Operators set ``EVSEProcessing_Ongoing_Count`` via the override
        # editor (default 0 = always Finished). The counter decrements
        # each round and we send "Finished" once it hits 0.
        self._cable_check_ongoing_remaining: int = 0
        self._charge_param_ongoing_remaining: int = 0
    ...
    def reInit(self) -> None:
        ...
        self.evccid = ""
        self.blChargeStopTrigger = False
        # Reset PreCharge ramp so the next session starts at 0 V.
        self._precharge_present_voltage = 0.0
        self._precharge_target_voltage = 0.0
        # Reset CableCheck Ongoing-counter (see _state_wait_flexible).
        self._cable_check_ongoing_remaining = 0
        # Reset ChargeParameter Ongoing-counter.
        self._charge_param_ongoing_remaining = 0
        if self.Tcp:
    ...
    def _state_wait_flexible(self) -> None:
        ...
        if "ChargeParameterDiscoveryReq" in decoded:
            # Initialise the Ongoing-counter the first time we enter
            # ChargeParameterDiscovery. Operators override
            # ``EVSEProcessing_Ongoing_Count`` via the Stage Config Panel
            # to make the EVSE reply with ``EVSEProcessing=Ongoing``
            # for that many rounds before flipping to Finished —
            # mirrors the behaviour of real chargers that need a few
            # seconds of internal startup before they advertise final
            # charge parameters.
            override = self.pause_controller.get_override(
                "ChargeParameterDiscoveryRes"
            ) or {}
            ongoing_total = int(override.get("EVSEProcessing_Ongoing_Count", 0))
            if (self._charge_param_ongoing_remaining == 0
                    and ongoing_total > 0):
                self._charge_param_ongoing_remaining = ongoing_total
            if self._charge_param_ongoing_remaining > 0:
                effective_processing = 1     # 1 = Ongoing in EDe
                self._charge_param_ongoing_remaining -= 1
            else:
                effective_processing = 0     # 0 = Finished
            ...
            def build_cpd(p: dict[str, Any]) -> str:
                parts = [
                    str(p.get("ResponseCode", 0)),
                    str(p.get("EVSEProcessing", effective_processing)),
                    ...
                    # Max power limit. Default 90 kW (200 A × 450 V) so
                    # the EV's charge profile maths is consistent with
                    # the current/voltage limits below — earlier the
                    # default 10 kW fought the 200 A / 450 V advertised
                    # values and some BMSes throttle to the lower.
                    str(p.get("EVSEMaximumPowerLimit_isUsed", 1)),
                    str(p.get("EVSEMaximumPowerLimitMultiplier", 3)),
                    str(p.get("EVSEMaximumPowerLimit", 90)),
        ...
        if "CableCheckReq" in decoded:
            # Same Ongoing-loop pattern as ChargeParameterDiscoveryRes:
            # operator override ``EVSEProcessing_Ongoing_Count`` lets
            # the EVSE reply N rounds of ``Ongoing`` before flipping
            # to ``Finished``. Real chargers do this while running
            # their isolation check (~5 s typical). Default 0 = always
            # Finished, matches HotWire's pre-fix behaviour.
            cc_override = self.pause_controller.get_override("CableCheckRes") or {}
            cc_ongoing_total = int(cc_override.get("EVSEProcessing_Ongoing_Count", 0))
            if (self._cable_check_ongoing_remaining == 0
                    and cc_ongoing_total > 0):
                self._cable_check_ongoing_remaining = cc_ongoing_total
            if self._cable_check_ongoing_remaining > 0:
                cc_processing = 1                   # 1 = Ongoing
                self._cable_check_ongoing_remaining -= 1
            else:
                cc_processing = 0                   # 0 = Finished
        ...
        if "PreChargeReq" in decoded:
            # Default simulation behaviour: ramp ``EVSEPresentVoltage``
            # from 0 V toward the EV's ``EVTargetVoltage`` over multiple
            # request/response rounds — same model real DC chargers
            # use to avoid contactor inrush. An operator override from
            # ``PauseController.set_override({"EVSEPresentVoltage": ...})``
            # wins outright (that's how ForcedDischarge keeps the lying
            # voltage flat instead of ramping).
            target_v = 350
            try:
                d = json.loads(decoded)
            ...
            # Ramp ``_precharge_present_voltage`` toward target_v. If the
            # EV changes its target between rounds (rare, but legal), we
            # adapt the ramp accordingly. Once we reach the target the
            # value stays pinned there — the PEV exits PreCharge in one
            # more PreChargeRes round.
            if target_v != self._precharge_target_voltage:
                self._precharge_target_voltage = float(target_v)
            if self._precharge_present_voltage < target_v:
                self._precharge_present_voltage = min(
                    self._precharge_present_voltage + _PRECHARGE_RAMP_STEP_V,
                    float(target_v),
                )
            elif self._precharge_present_voltage > target_v:
                # Defensive — shouldn't normally happen because we only
                # ramp upward, but if the EV lowers its target mid-PreCharge
                # we follow it down at the same rate.
                self._precharge_present_voltage = max(
                    self._precharge_present_voltage - _PRECHARGE_RAMP_STEP_V,
                    float(target_v),
                )
            ramped_present_v = int(self._precharge_present_voltage)
            ...
            self._intercept_and_send(
                "PreChargeRes",
                {
                    "EVSEPresentVoltage": ramped_present_v,
                    "ResponseCode": 0,              # OK
                    "IsolationStatusUsed": 1,
                    "IsolationStatus": 1,           # Valid
                    "EVSEStatusCode": 1,            # EVSE_Ready
    ...
    def _state_stopped(self) -> None:
        if self.rxData:
            ...
            self.rxData = b""
        # After a brief settle window, auto-reinit so the EVSE goes
        # back to ``WAIT_APP_HANDSHAKE`` and a freshly-plugged vehicle
        # can complete a new session without the operator clicking
        # Reset FSM. Matches real charging-station behaviour: a
        # commercial CCS station resets back to "Ready" after every
        # session terminus and waits for the next vehicle.
        if self.cyclesInState >= 166:                  # ~5 s @ 30 ms TICK
            self.addToTrace("Stopped state settled — auto-reinitializing for next session")
            self.reInit()
