...
Timeout-check pattern: every state handler uses an *independent*
``if self.isTooLong()`` (not ``elif``) after rxData processing — matches
pyPLC's behaviour and ensures a stream of garbage / malformed inbound
frames cannot silently keep the FSM alive past its spec window.
...
class fsmPev:
    ...
    def isTooLong(self) -> bool:
        ...
        * Phase-specific wall-clock timers (Table 78): CableCheck 38 s,
        ...
        from .din_spec import (
            ...
            V2G_EVCC_PRE_CHARGE_TIMEOUT_LENIENT_S,
        )
        ...
        # PreCharge phase timeout. DIN spec says 7 s but pyPLC's
        # field-tested value is 30 s — real EVSE DC ramp-up regularly
        # takes longer than 7 s to reach battery voltage on commercial
        # chargers. We use the lenient value so paper-validation runs
        # against live hardware don't false-timeout while the EVSE is
        # still ramping; conformance tests reference V2G_EVCC_PRE_CHARGE_TIMEOUT_S
        # directly for the spec value.
        elif self.state == STATE_WAIT_PRECHARGE_RES:
            limit = seconds_to_cycles(V2G_EVCC_PRE_CHARGE_TIMEOUT_LENIENT_S)
    ...
    def _state_wait_app_res(self) -> None:
        ...
        if self.isTooLong():    # independent — matches pyPLC; see top of file
    ...
    def _state_wait_precharge_res(self) -> None:
        ...
        if self.rxData:
            ...
            if "PreChargeRes" in decoded:
                ...
                # PreCharge-completion reference. In real-PEV semantics
                # this is the actual battery voltage (accu_v from
                # ``charge_target_voltage`` config); PreCharge ends when
                # the EVSE has ramped its DC pins to match the battery.
                #
                # In testbed mode, however, the operator may have
                # overridden ``EVTargetVoltage`` to e.g. 500 V via the
                # PEV.PreChargeReq Override editor (or via an Attack
                # playbook). They expect "I asked for 500, EVSE confirms
                # 500, PreCharge done" — but a fixed accu_v=220 V would
                # never let PreCharge complete. Honour the override:
                # if there's an active EVTargetVoltage override on
                # PreChargeReq, use it as the reference instead of the
                # static accu_v. Both real-vehicle work (no override =>
                # uses accu_v) and testbed attack scripts (override =>
                # uses the override value) get the right behaviour.
                accu_v = self.hardwareInterface.getAccuVoltage()
                try:
                    pcr_override = self.pause_controller.get_override("PreChargeReq")
                except Exception:                                       # noqa: BLE001
                    pcr_override = None
                if pcr_override and "EVTargetVoltage" in pcr_override:
                    try:
                        accu_v = float(pcr_override["EVTargetVoltage"])
    ...
    def _state_charging_finished(self) -> None:
        ...
        # Mark the End-state arrival as "normal completion". The
        # ``_state_end`` handler reads this flag to decide whether to
        # auto-reinit (transient error recovery) or stay idle (normal
        # end — operator has to physically re-plug to start another
        # session, exactly like a real EV).
    ...
    def _state_safe_shutdown_wait_contactors(self) -> None:
        ...
        self.hardwareInterface.triggerConnectorUnlocking()
        # Mark the End-state arrival as "abnormal" so ``_state_end``
        # auto-reinits — this path runs after SequenceTimeout or
        # UnrecoverableError, both transient conditions where the
        # operator likely wants HotWire to re-arm and reach for a
        # fresh session on the next plug-in.
        self._end_reason = "abnormal"
        self.enterState(STATE_END)

    def _state_end(self) -> None:
        """Behaviour depends on **why** we got here:

        * Normal completion (``ChargingFinished`` → End): stay idle
          forever. Real EVs don't auto-restart a charging session — the
          driver has to physically unplug + replug. The HotWire operator
          presses Reset FSM (or Stop+Start) when they want another run.
          Auto-reconnecting here was the cause of the user-reported "PEV
          re-connects after stop" loop — the FSM kept hammering the EVSE
          with new TCP connects every 5 s long after the session was
          legitimately complete.

        * Abnormal exit (``SafeShutdown`` after ``SequenceTimeout`` /
          ``UnrecoverableError``): auto-reinit after a 5 s settle. These
          paths suggest a transient hiccup — momentary EVSE silence,
          one bad EXI frame — and the operator generally wants to retry
          rather than manually intervene.

        ``self._end_reason`` is set by whichever predecessor state
        transitioned here (``ChargingFinished`` vs
        ``SafeShutdownWaitContactors``). Default is ``"abnormal"`` so
        previously-deployed code paths that don't set the flag still
        recover.
        """
        reason = getattr(self, "_end_reason", "abnormal")
        if reason == "normal":
            # Stay idle. Real EV behaviour after a legitimate session
            # end. Operator clicks Reset FSM / Stop+Start to start
            # another session.
            return
        if self.cyclesInState < 166:
            return
        self.addToTrace("End state settled — auto-reinitializing for next session")
        self.reInit()
