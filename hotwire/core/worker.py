...
class HotWireWorker:
    ...
    def shutdown(self) -> None:
        ...
        then disconnect the PEV TCP client (if any), drop FSM refs,
        finally release the lower layers (pcap + Arduino serial).
        """
        ...
        self.evse = None
        self.pev = None
        # Release the lower layers — pcap interface (RealHomePlug) and
        # Arduino serial (RealHardwareInterface). Without these the next
        # GUI Start gets a "device busy" failure or a stale SLAC handle.
        # Both close() implementations are no-ops on the simulated
        # variants so this is safe to call unconditionally.
        if self.hp is not None:
            try:
                self.hp.close()
            except AttributeError:
                # Older hp drivers without close() — skip silently.
                pass
            except Exception as e:                                  # noqa: BLE001
                self._worker_trace(f"[WORKER] hp.close raised: {e}")
            self.hp = None
        if self.hardwareInterface is not None:
            try:
                self.hardwareInterface.close()
            except AttributeError:
                pass
            except Exception as e:                                  # noqa: BLE001
                self._worker_trace(
                    f"[WORKER] hardwareInterface.close raised: {e}"
                )
