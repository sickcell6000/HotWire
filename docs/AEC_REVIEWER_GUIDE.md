...
- **F1** pytest regression — 278 unit + integration tests via
  ...
  tests auto-skip).
- **F2** Simulation-mode full V2G session on `::1` loopback
  (~25 s end-to-end), expects 13 PEV state transitions and CurrentDemand
  cycles.
- **F3** Parametric matrix: 3 voltages × 3 durations = 9 successful
  sim-mode sessions.
- **F4** Attack code present and syntactically valid — `py_compile`
  for both `autocharge_impersonation.py` (A1) and
  `forced_discharge.py` (A2).
- **F5** Sim-mode attack reach — runs three end-to-end scenarios
  (A1 EVCCID injection, A2 forged PreCharge voltage, A1+A2
  concurrent) and asserts the fabricated values land on the wire.
- **F6** Real-hardware evidence bundles — validates the 11 frozen
  pcap files in `datasets/real_hw_traces/` open as pcap v2.4.
